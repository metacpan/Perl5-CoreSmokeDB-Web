...
<button
 ...
 style="margin: 1rem"
 @click="openFile('outfile', reportId)">Show outfile</button>
<button
 v-if="hasLength(rd.log_file)"
 style="margin: 1rem"
 @click="openFile('logfile', reportId)">Show logfile</button>
...
export default {
  ...
  setup() {
    ...
    const openFile = function (type, id) {
      const fileType = type === 'logfile' ? 'logfile' : 'outfile';
  ...
  created() {
    if (!this.hasFullReport) {
      this.getFullSmokeReport(this.reportId);
    }
  },
  methods: {
    ...mapActions({
    ...
    async getFullSmokeReport(reportId) {
      this.isLoading = true;
      ...
      try {
        ...
        console.log(e);
        // window.alert("Error fetching report " + e);
      };
